...
@Component(//
		name = "Sennec.HomeV3.Virtual.Meter.Singlephase", //
		immediate = true, //
		configurationPolicy = ConfigurationPolicy.REQUIRE //
)
public class MeterSennecSinglephase extends AbstractOpenemsComponent
		implements SinglePhaseMeter, ElectricityMeter, OpenemsComponent {

	protected Config config = null;
	protected MeterType meterType = MeterType.PRODUCTION;

	private final Logger log = LoggerFactory.getLogger(MeterSennecSinglephase.class);
	...
	// protected void updateOutChannelLong(Channel<Long> outChannel, Value<Long>
	// newValue) {
	// Optional<Long> valueOpt = (Optional<Long>) newValue.asOptional();
	// if (!valueOpt.isPresent()) {
	// outChannel.setNextValue(null);
	// return;
	// }
	// Long activePower = TypeUtils.getAsType(OpenemsType.LONG, newValue);
	//// if(outChannel == null || outChannel.getType() == null) {
	//// log.error("No long out channel");
	//// }else {
	//// log.warn("Channel " + outChannel.toString());
	// outChannel.setNextValue(activePower);
	//// }
	// }
	//
